...
//                .fetchOne(); // 여러개 조회시 오류
    ...
    public void sort() {
        ...
        assertThat(findMember.get(2).getUsername()).isEqualTo(null);
    }
    ...
    public void join() {
        ...
        assertThat(result)
                .extracting("username") // extracting : 특정 필드 추출 (username)
    ...
    public void join_on_no_relation() {
        ...
        // join 테이블이 연관관계(member.team) 가 없는 테이블로 할 경우 id 가 같은지 여부는 상관하지 않고 조건에만 맞는 값을 조회한다.
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name))  // (member.team 과 team의 id가 달라도 조건만 맞으면 조회)
                .fetch();
    ...
    /**
     * member를 조회하지만 반환값은 Dto 로 할때
     * 2. Querydsl property (setter) ver
     */
    @Test
    ...
    /**
     * member를 조회하지만 반환값은 Dto 로 할때
     * 3. Querydsl field ver (getter, setter 가 없어도 값이 주입됨)
     */
    @Test
    ...
    /**
     * member를 조회하지만 반환값은 Dto 로 할때
     * 4. Querydsl constructor ver (생성자로 값 주입)
     */
    @Test
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
//                .where(allEq(usernameCond, ageCond)) // 메서드 조립하여 사용 가능
